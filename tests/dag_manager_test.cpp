...
protected:
  void SetUp() override { dag_manager_ = std::make_unique<DAGManager>(); }

  void TearDown() override { dag_manager_.reset(); }
...
TEST_F(DAGManagerTest, CreateDag) {
  ...
  auto info = test::create_simple_dag_info("Test DAG");
...
TEST_F(DAGManagerTest, ListDags) {
  ASSERT_TRUE(
      dag_manager_
          ->create_dag(DAGId{"dag1"}, test::create_simple_dag_info("DAG 1"))
          .has_value());
  ASSERT_TRUE(
      dag_manager_
          ->create_dag(DAGId{"dag2"}, test::create_simple_dag_info("DAG 2"))
          .has_value());
  ASSERT_TRUE(
      dag_manager_
          ->create_dag(DAGId{"dag3"}, test::create_simple_dag_info("DAG 3"))
          .has_value());
...
TEST_F(DAGManagerTest, DeleteDag) {
  DAGId dag_id{"test_dag"};
  ASSERT_TRUE(
      dag_manager_->create_dag(dag_id, test::create_simple_dag_info("Test"))
          .has_value());
...
TEST_F(DAGManagerTest, GetTaskFromDag) {
  ...
  ASSERT_TRUE(
      dag_manager_
          ->create_dag(dag_id, test::create_dag_info_with_task("Test", task_id))
          .has_value());
...
TEST_F(DAGManagerTest, WouldCreateCycleSimple) {
  ...
  task2.dependencies = {
      TaskDependency{.task_id = TaskId{"task1"}, .label = ""}};

  TaskConfig task3;
  task3.task_id = TaskId{"task3"};
  task3.dependencies = {
      TaskDependency{.task_id = TaskId{"task2"}, .label = ""}};

  info.tasks = {task1, task2, task3};
  ASSERT_TRUE(dag_manager_->create_dag(dag_id, info).has_value());

  auto result = dag_manager_->would_create_cycle(dag_id, TaskId{"task1"},
                                                 {TaskId{"task3"}});
  EXPECT_TRUE(result);
}
...
TEST_F(DAGManagerTest, AddTaskWithInvalidDependencyFails) {
  ...
  task.command = "echo hello";
  task.dependencies = {
      TaskDependency{.task_id = TaskId{"nonexistent"}, .label = ""}};
...
TEST_F(DAGManagerTest, WouldCreateCycle_ThreeNodeCycle) {
  ...
  task2.dependencies = {TaskDependency{.task_id = TaskId{"a"}, .label = ""}};

  TaskConfig task3;
  task3.task_id = TaskId{"c"};
  task3.dependencies = {TaskDependency{.task_id = TaskId{"b"}, .label = ""}};

  info.tasks = {task1, task2, task3};
  ASSERT_TRUE(dag_manager_->create_dag(dag_id, info).has_value());

  EXPECT_TRUE(
      dag_manager_->would_create_cycle(dag_id, TaskId{"a"}, {TaskId{"c"}}));
  EXPECT_FALSE(
      dag_manager_->would_create_cycle(dag_id, TaskId{"c"}, {TaskId{"a"}}));
}
...
TEST_F(DAGManagerTest, WouldCreateCycle_DiamondDependency) {
  ...
  left.dependencies = {TaskDependency{.task_id = TaskId{"start"}, .label = ""}};

  TaskConfig right;
  right.task_id = TaskId{"right"};
  right.dependencies = {
      TaskDependency{.task_id = TaskId{"start"}, .label = ""}};

  TaskConfig end;
  end.task_id = TaskId{"end"};
  end.dependencies = {{.task_id = TaskId{"left"}, .label = ""},
                      {.task_id = TaskId{"right"}, .label = ""}};

  info.tasks = {start, left, right, end};
  ASSERT_TRUE(dag_manager_->create_dag(dag_id, info).has_value());

  EXPECT_TRUE(dag_manager_->would_create_cycle(dag_id, TaskId{"start"},
                                               {TaskId{"end"}}));
  EXPECT_FALSE(dag_manager_->would_create_cycle(dag_id, TaskId{"left"},
                                                {TaskId{"right"}}));
}
