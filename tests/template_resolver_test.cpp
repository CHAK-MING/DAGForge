...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        DateTemplateTestCase{"ds_basic", "date={{ds}}", "date=2026-01-15"},
        DateTemplateTestCase{"ds_nodash_basic", "date={{ds_nodash}}",
                             "date=20260115"},
        DateTemplateTestCase{"ts_basic", "time={{ts}}",
                             "time=2026-01-15T10:30:45Z"},
        DateTemplateTestCase{"ts_nodash_basic", "time={{ts_nodash}}",
                             "time=20260115T103045"},
        DateTemplateTestCase{"multiple_ds", "from={{ds}}_to={{ds}}",
                             "from=2026-01-15_to=2026-01-15"},
        DateTemplateTestCase{"mixed_variables", "{{ds}}/{{ds_nodash}}/{{ts}}",
                             "2026-01-15/20260115/2026-01-15T10:30:45Z"},
        DateTemplateTestCase{"no_variables", "plain text", "plain text"},
        DateTemplateTestCase{"partial_match_not_replaced", "{{ds_invalid}}",
                             "{{ds_invalid}}"},
        DateTemplateTestCase{"embedded_in_path", "/data/{{ds_nodash}}/file.csv",
...
TEST_F(DateTemplateEdgeCaseTest, YearBoundary_December31) {
  ...
  ASSERT_TRUE(result.has_value());
  EXPECT_EQ(*result, "2025-12-31 2025-12-31T23:59:59Z");
}
...
TEST_F(DateTemplateEdgeCaseTest, MixedEscapedAndNormal) {
  ...
  auto result =
      resolver_->resolve_template(R"(date={{ds}} literal=\{{ds}})", ctx, {});
  ASSERT_TRUE(result.has_value());
...
TEST_F(DateTemplateEdgeCaseTest, DataIntervalStart_Resolves) {
  std::tm tm{};
  tm.tm_year = 2026 - 1900;
  tm.tm_mon = 0;
  tm.tm_mday = 15;
...
TEST_F(DateTemplateEdgeCaseTest, DataIntervalEnd_Resolves) {
  std::tm tm_start{};
  tm_start.tm_year = 2026 - 1900;
  tm_start.tm_mon = 0;
  tm_start.tm_mday = 15;
  tm_start.tm_hour = 0;
  auto start = system_clock::from_time_t(timegm(&tm_start));
  ...
  ASSERT_TRUE(result.has_value());
  EXPECT_EQ(*result, "2026-01-16T00:00:00Z");
}

TEST_F(DateTemplateEdgeCaseTest, DataInterval_EmptyLeavesLiteral) {
  std::tm tm{};
  tm.tm_year = 2026 - 1900;
  tm.tm_mon = 0;
  tm.tm_mday = 15;
  ...
  auto result = resolver_->resolve_template("{{data_interval_start}}", ctx, {});
  ASSERT_TRUE(result.has_value());
  EXPECT_EQ(*result, "{{data_interval_start}}");
}

TEST_F(DateTemplateEdgeCaseTest, DataInterval_MixedWithOtherVariables) {
  std::tm tm{};
  tm.tm_year = 2026 - 1900;
  tm.tm_mon = 0;
  tm.tm_mday = 15;
  ...
  ASSERT_TRUE(result.has_value());
  EXPECT_EQ(*result, "2026-01-15 2026-01-15T10:30:00Z 2026-01-16T10:30:00Z");
}

TEST_F(DateTemplateEdgeCaseTest, DataIntervalStart_WhitespaceTrimmed) {
  std::tm tm{};
  tm.tm_year = 2026 - 1900;
  tm.tm_mon = 0;
  tm.tm_mday = 15;
