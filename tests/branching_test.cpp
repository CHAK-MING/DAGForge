...
TEST_F(BranchingTest, IsBranchTask_ReturnsTrueForBranchNodes) {
  auto idx =
      dag_.add_node(TaskId("branch_task"), TriggerRule::AllSuccess, true);
...
TEST_F(BranchingTest, IsBranchTask_ReturnsFalseForNormalNodes) {
  auto idx =
      dag_.add_node(TaskId("normal_task"), TriggerRule::AllSuccess, false);
  ASSERT_TRUE(idx.has_value());
...
TEST_F(BranchingTest, MarkCompletedWithBranch_SkipsUnselectedDownstream) {
  ...
  auto run_result =
      DAGRun::create(DAGRunId("run1"), std::make_shared<DAG>(dag_));
  ASSERT_TRUE(run_result.has_value());
  auto &run = *run_result;

  ASSERT_TRUE(run.mark_task_started(*branch, InstanceId("inst1")));
...
TEST_F(BranchingTest, MarkCompletedWithBranch_EmptySelectionDoesNotSkip) {
  ...
  ASSERT_TRUE(run.mark_task_completed_with_branch(*branch, 0, empty_selection));
...
TEST_F(BranchingTest, MarkCompletedWithBranch_NonBranchTaskIgnoresSelection) {
  ...
  ASSERT_TRUE(run.mark_task_started(*normal, InstanceId("inst1")));

  std::array<TaskId, 1> selected{TaskId("other")};
  ASSERT_TRUE(run.mark_task_completed_with_branch(*normal, 0, selected));
