...
TEST_F(DockerExecutorTest, ExecutorHandlesInvalidConfig) {
  ...
  ExecutionSink sink{
      ...
      .on_stderr = nullptr,
      .on_complete =
          [&](InstanceId, ExecutorResult result) {
            final_result = std::move(result);
            completed = true;
          },
  };

  executor_->start(std::move(req), std::move(sink));
...
TEST_F(DockerExecutorTest, ExecutorTypeRegistration) {
  auto type = parse<ExecutorType>("docker");
  EXPECT_EQ(type, ExecutorType::Docker);

  auto str = to_string_view(ExecutorType::Docker);
